...
def OwnScore(sEnergy_Bond_dict, allPeaks_list, current_mol, bBreakRing, precursor_type, bRankSum) :
#    global dMass_Tolerance_Fragment_Ions
#    dMass_Tolerance_Fragment_Ions = 0.01
    ...
    total_fragment_list, observed_fragment_list, all_compound_match  = ExhaustBonds(current_mol, allPeaks_list, peakmatch_list, sEnergy_Bond_dict, bBreakRing,precursor_type)
...
def TreeLikeBreakBondsDepthFirst(current_mol, iBondsNum, allPeaks_list, iDepth, peakmatch_list, sEnergy_Bond_dict, energy_denominator, bBreakRing, precursor_type, total_fragment_list, observed_fragment_list, all_compound_match) :
    ...
    while (len(storedNodes) > 0) :
        if (len(storedNodes[-1][3]) > 0) : # unprocessed kid
            new_item = processKid(storedNodes[-1][3][0][0], storedNodes[-1][3][0][1], storedNodes[-1][0][2]+1, allPeaks_list, peakmatch_list, sEnergy_Bond_dict, energy_denominator, bBreakRing, precursor_type, total_fragment_list, observed_fragment_list, all_compound_match) 
...
def processKid(current_editable_mol, current_removebond_list, iCurrent_depth, allPeaks_list, peakmatch_list, sEnergy_Bond_dict, energy_denominator, bBreakRing, precursor_type, total_fragment_list, observed_fragment_list, all_compound_match) :
    ...
    bFindPeak, dCurrentFragmentBDE = MapMass(current_dMass, allPeaks_list, peakmatch_list, current_sFragmentFormula, current_smiles, current_removebond_list, sEnergy_Bond_dict, energy_denominator, precursor_type, total_fragment_list, observed_fragment_list, iCurrent_depth)
...
#    print Descriptors.ExactMolWt(current_mol), "NULL", AllChem.CalcMolFormula(current_mol)
    bFindPeak, dCurrentFragmentBDE  = MapMass(Descriptors.ExactMolWt(current_mol), allPeaks_list, peakmatch_list, current_sFragmentFormula, current_smiles, [],sEnergy_Bond_dict, energy_denominator, precursor_type, total_fragment_list,  observed_fragment_list, 0)
    all_compound_match.append([bFindPeak, dCurrentFragmentBDE])
    iBondsNum = current_mol.GetNumBonds()
    #TreeLikeBreakBonds(current_mol, iBondsNum, allPeaks_list, 3, peakmatch_list)
    TreeLikeBreakBondsDepthFirst(current_mol, iBondsNum, allPeaks_list, 3, peakmatch_list, sEnergy_Bond_dict, energy_denominator, bBreakRing, precursor_type, total_fragment_list, observed_fragment_list, all_compound_match)
