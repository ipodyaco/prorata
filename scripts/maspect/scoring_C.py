...
def score_main(Compound_list, sOutput_Filename, sEnergy_Bond_dict,bSpectrumDetails, bBreakRing, dCurrentPrecursor_type, bRankSum, dCurrentParentMass, current_peaks_list, sCurrentScanNumber, mylock, iParentMassWindow_list, dMass_Tolerance_Parent_Ion, dMass_Tolerance_Fragment_Ions, iFragmentation_Depth, sFT2_basename, iPositive_Ion_Fragment_Mass_Windows_list, iNegative_Ion_Fragment_Mass_Windows_list):
    ...
    for each_compound in QueryCompound_list :
       ...
       # print len(QueryCompound_list)
        dCurrentScore, dCurrentEnergy, iIdentifiedPeak, sAnnotation_list, sOtherInfo= weightedscore.OwnScore(sEnergy_Bond_dict, allPeaks_list, current_mol, bBreakRing, dCurrentPrecursor_type, bRankSum, dMass_Tolerance_Fragment_Ions, iFragmentation_Depth, iParentMassWindow_list, iPositive_Ion_Fragment_Mass_Windows_list, iNegative_Ion_Fragment_Mass_Windows_list)
        if (dCurrentPrecursor_type == 1) :
            sScanType = "positive ion mode"
        else :
            sScanType = "negative ion mode"
        Compound_Scores_list.append([dCurrentScore, dCurrentEnergy, each_compound[0], each_compound[1], each_compound[3], iIdentifiedPeak, sOtherInfo, sAnnotation_list, sScanType, each_compound[2], each_compound[4]])
    ...
    #    print sCurrentScanNumber, i 
        ...
        output_str += sFT2_basename + "\t" + sCurrentScanNumber + "\t" + sScanType + "\t" + str(i+1) + "\t" + str(dParentMassError) + "\t"
