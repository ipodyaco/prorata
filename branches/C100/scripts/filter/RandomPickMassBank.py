...
def filterMassBankFile(input_filename, output_file, badcase_dir) :

    dPrecursorEM, sPeak_list, sInichi, sPubChemId, sAccession = ReadRealHit(input_filename)

    if (sPubChemId == "") :
        os.system("mv "+input_filename+" "+badcase_dir)
    else:
...
def main(argv=None):
    ...
    if argv is None:
        ...
        for each_input_filename in input_filename_list :
            current_random_number = random.randint(1,8)
            if (current_random_number%8 == 0) :
                filterMassBankFile(each_input_filename, output_file, badcase_dir)
            else :
                os.system("rm "+each_input_filename)
