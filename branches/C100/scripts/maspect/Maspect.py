...
def parse_options (argv) :
    opts, args = getopt.getopt(argv[1:], "hf:c:o:",
                                ["help", "ft2-filename", "configure-filename", "output-filename"])
    sFT2_Filename      = ""
    sOutput_Filename    = ""
    sConfigure_Filename = ""
    ...
    for option, value in opts:
        if option in ("-h", "--help"):
            print "-f ft2-filename -c configure-filename -o output-filename"
            sys.exit(0)
    ...
    return [sFT2_Filename,  sOutput_Filename, sConfigure_Filename]
...
def HandleAllRealHits(all_scans_list, compound_filename, sOutput_Filename, sEnergy_Bond_dict, process_number, bSpectrumDetails, bBreakRing, bRankSum, iParentMassWindow_list, dMass_Tolerance_Parent_Ion, dMass_Tolerance_Fragment_Ions, iFragmentation_Depth, sFT2_basename, iPositive_Ion_Fragment_Mass_Windows_list, iNegative_Ion_Fragment_Mass_Windows_list) :
...
def OutputFileHead(sConfigure_Filename) :
    ...
    sOutputhead += "Filename\tScanNumber\tScanType\tRank\tParentMassError\tExplainedPeaks\tScore\tIdentifier\tName\tOriginalInChI\tIdentifiedInChI\tModifications\tLinks\n"
    ...
    for each_line in FT2_File :
        ...
        if not(each_line[0].isdigit()) :
            if (current_peaks_list) :
                ...
                all_scans_list.append([current_peaks_list, dPrecursorMofZ - precursor_type*dProtonMass, sScanNumber, precursor_type])
                current_peaks_list = []
...
def main(argv=None):
    ...
    if argv is None:
        argv = sys.argv
       		 # parse options
        [sFT2_Filename, sOutput_Filename, sConfigure_Filename] = parse_options(argv)
...
#        print all_scans_list
        ...
        #print iParentMassWindow_list, iPositive_Ion_Fragment_Mass_Windows_list, iNegative_Ion_Fragment_Mass_Windows_list
        HandleAllRealHits(all_scans_list, compound_filename, sOutput_Filename, sEnergy_Bond_dict, process_number, bSpectrumDetails, bBreakRing, bRankSum, iParentMassWindow_list, dMass_Tolerance_Parent_Ion, dMass_Tolerance_Fragment_Ions, iFragmentation_Depth, sFT2_basename, iPositive_Ion_Fragment_Mass_Windows_list, iNegative_Ion_Fragment_Mass_Windows_list)
