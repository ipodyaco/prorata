...
def ReadRealHit(realhit_filename):
    ...
    if ((sPrecursorType == "[M-H]-") or (sPrecursorType == "NEGATIVE")):
        ...
        if ((massdiff >= 0.01) and (sMSType == "MS2")):
            print realhit_filename, massdiff, "-1"
    return precursor_mz - current_mode*dProtonMass, allPeaks_list, s_chemical_structure, sPubChemId, sAccession, sMSType, bLegalStructure, sPrecursorType, current_mode, sCompoundName, massdiff

def extractMassBankFile(input_filename, output_file, mode) :

    dPrecursorEM, sPeak_list, sInichi, sPubChemId, sAccession, sMSType, bLegalStructure, sPrecursorType, current_mode, sCompoundName, massdiff = ReadRealHit(input_filename)
...
#        print bLegalStructure
    ...
    if (sCompoundName == "") :
        sCompoundName = "NA"
    if ((sMSType == "MS2") and (bLegalStructure)and (mode*current_mode == 1) and (massdiff <= 0.01)):
        if (dPrecursorEM < 0) :
