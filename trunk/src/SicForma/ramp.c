     ...
     *iLastScan = 0;
     free(pScanIndex);
      if ((indexOffset < 0) || (retryLoop==0)) { // derive the index by inspection
         ...
         buf[sizeof(buf)-1] = 0;
         while (ramp_fread(buf,sizeof(buf)-1,pFI)>taglen) {
            char *find;
            char *look=buf;
            int nread;
            while (find = strstr(look,scantag)) {
...
#define LOWMZ_UNINIT 1.111E6
   ...
   if (pFI->bIsMzData) {
      ...
      while (ramp_fgets(stringBuf, SIZE_BUF, pFI))
      {
         ...
         for (attrib=stringBuf-1;attrib=strchr(attrib+1,'=');) {
   ...
   if (pFI->bIsMzData) {
      ...
      while ((!(bGotInten && bGotMZ)) &&
         ...
         if (isArray) {
            ...
            if ((pDecoded = (char *) malloc(peaksCount * (precision/8) + 1)) == NULL)
            {
               printf("Cannot allocate memory\n");
               return NULL;
            }
            ...
            // warning - there's an implicit intel byte order assumption here -
            // have to get fancier if you want to run this code on motorola chips
            if (32==precision) {
               for (n = 0; n < peaksCount; n++) {
                  uint32_t tmp = isLittleEndian?((uint32_t *) pDecoded)[n]:
                     ntohl( ((uint32_t *) pDecoded)[n]);
                  pPeaks[isInten+(2*n)] = (RAMPREAL) (* (float *)(&tmp));
               }
             } else {
               for (n = 0; n < peaksCount; n++) {
                  uint64_t tmp = isLittleEndian?((uint64_t *) pDecoded)[n]:
                     ntohll( ((uint64_t *) pDecoded)[n]);
      ...
      if (32==precision) {
         ...
         {
            uint64_t tmp =
               ntohll((uint64_t) ((uint64_t *) pDecoded)[n]);
